...
int prefix_lcs_via_braid_bits_4symbol_splited_a_one_b_arbitrary(Input *a_reverse, int a_size, int a_total_symbols,
    ...
    for (int k = std::max(0, total_same_length_diag - 1); k < total_same_length_diag; ++k) {
        ...
        loop_center_half_with_mask(0, 1, 0, k + 1, braid_ones,
                                   Input(l_active_mask & r_active_mask), bitset_left_strand_map, bitset_top_strand_map,
                                   a_reverse, b);
...
int prefix_lcs_via_braid_4symbol_one_one_size(Input a_reverse, int a_total_symbols, Input b, int b_total_symbols) {
    ...
    for (int inside_diag_num = 0; inside_diag_num <= upper_bound; ++inside_diag_num, rev_counter -= 2) {
        ...
        if (combing_condition) {
            ...
            symbols &= (symbols >> 1) & braid_ones;
            combing_condition =
                    l_active_mask & (r_active_mask << rev_counter) & mask_r & (symbols | ((~(left_strand) & old_top)));
            rev_combing_cond = combing_condition ^ braid_ones;
    ...
    for (int inside_diag_num = 0; inside_diag_num < upper_bound; ++inside_diag_num) {
        ...
        Input combing_condition = (l_active_mask << (2 * (inside_diag_num + 1))) & r_active_mask & mask &
        ...
        if (combing_condition) {
            ...
            symbols &= (symbols >> 1) & braid_ones;
            combing_condition = l_active_mask & (r_active_mask >> (2 * (inside_diag_num + 1))) & mask_r &
                                (symbols | ((~(left_strand) & old_top)));
            rev_combing_cond = combing_condition ^ braid_ones;
            left_strand = (rev_combing_cond & left_strand) | (combing_condition & old_top);
        }
    }
...
int prefix_lcs_via_braid_bits_4symbol(Input *a_reverse, int a_size, int a_total_symbols,
    ...
    if (active_symbols_a_active == 0 && active_symbols_b_active == 0) {
        //multiple case
        return prefix_lcs_via_braid_bits_4symbol_splited(a_reverse, a_size, a_total_symbols, b, b_size,
                                                         b_total_symbols);
        ...
        return prefix_lcs_via_braid_bits_4symbol_splited_a_equals_b(a_reverse, a_size, a_total_symbols, b, b_size,
                                                                    b_total_symbols);
    }
    return prefix_lcs_via_braid_bits_4symbol_splited_a_less_b(
            a_reverse, a_size, a_total_symbols, b, b_size, b_total_symbols);
}
...
template<class Input>
int prefix_lcs_via_braid_bits_4symbol_splited_a_equals_b_mpi(Input *a_reverse, int a_size, int a_total_symbols,
                                                             Input *b, int b_size, int b_total_symbols,
                                                             int threads_num) {
...
#pragma omp parallel num_threads(threads_num)  default(none) shared(bitset_left_strand_map, bitset_top_strand_map, a_reverse, b, m, n, dis_braid, braid_ones, l_active_mask, r_active_mask)
    {
...
#pragma omp single nowait
                {
                    loop_upper_half(0, 1, rev_counter, m - 1 - cur_diag_cube_len, 0,
                                    braid_ones, Input(mask & (l_active_mask >> rev_counter)),
                                    Input(mask_r & l_active_mask),
                                    bitset_left_strand_map, bitset_top_strand_map, a_reverse, b);
                }
...
#pragma omp single nowait
                {
                    loop_upper_half(cur_diag_cube_len, cur_diag_cube_len + 1, rev_counter, m - 1 - cur_diag_cube_len, 0,
                                    braid_ones, Input(mask & r_active_mask),
                                    Input(mask_r & (r_active_mask << rev_counter)),
                                    bitset_left_strand_map, bitset_top_strand_map, a_reverse, b);
                }

                loop_upper_half_mpi(1, cur_diag_cube_len, rev_counter, m - 1 - cur_diag_cube_len, 0,
                                    braid_ones, mask, mask_r, bitset_left_strand_map, bitset_top_strand_map, a_reverse,
                                    b);
//tod top edge
                loop_lower_half_mpi(0, cur_diag_cube_len, inside_diag_num, m - 1 - cur_diag_cube_len + 1, 0,
...
#pragma omp single nowait
            ...
            loop_center_half_mpi(1, cur_diag_cube_len, m - 1 - cur_diag_cube_len, 0, braid_ones, bitset_left_strand_map,
                                 bitset_top_strand_map, a_reverse, b);
        }
...
#pragma omp single nowait
                ...
                loop_upper_half_mpi(1, cur_diag_cube_len - 1, rev_counter, 0, start_j, braid_ones, mask, mask_r,
                                    bitset_left_strand_map, bitset_top_strand_map, a_reverse, b);
...
#pragma omp single nowait
                {
                    loop_lower_half(cur_diag_cube_len, cur_diag_cube_len + 1, inside_diag_num, 0, start_j - 1,
                                    braid_ones,
                                    Input(mask_prev & r_active_mask),
                                    Input(mask_prev_r & (r_active_mask >> (2 * (inside_diag_num + 1)))),
                                    bitset_left_strand_map, bitset_top_strand_map, a_reverse, b);
                ...
                loop_lower_half_mpi(1, cur_diag_cube_len, inside_diag_num, 0, start_j - 1, braid_ones, mask_prev,
                                    mask_prev_r,
                                    bitset_left_strand_map, bitset_top_strand_map, a_reverse, b);
...
#pragma omp single nowait
            {
                loop_center_half_with_mask(cur_diag_cube_len - 1, cur_diag_cube_len, 0, start_j, braid_ones,
                                           r_active_mask,
                                           bitset_left_strand_map, bitset_top_strand_map, a_reverse, b);
            }
            loop_center_half_mpi(1, cur_diag_cube_len - 1, 0, start_j, braid_ones, bitset_left_strand_map,
                                 bitset_top_strand_map,
                                 a_reverse, b);
        }
        ...
        for (int cur_diag_cube_len = 1; cur_diag_cube_len >= 1; cur_diag_cube_len--, start_j++) {
            ...
            for (int inside_diag_num = 0; inside_diag_num < upper_bound; ++inside_diag_num, rev_counter -= 2) {
                ...
                loop_upper_half_mpi(0, cur_diag_cube_len, rev_counter, 0, start_j, braid_ones,
                                    Input(mask & r_active_mask & (l_active_mask >> rev_counter)),
                                    Input(mask_r & l_active_mask & (r_active_mask << rev_counter)),
                                    bitset_left_strand_map, bitset_top_strand_map, a_reverse, b);
...
#pragma omp single
            {
                loop_center_half_with_mask(0, cur_diag_cube_len, 0, start_j, braid_ones,
                                           Input(l_active_mask & r_active_mask),
                                           bitset_left_strand_map, bitset_top_strand_map, a_reverse, b);
            }
        }
...
template<class Input>
int prefix_lcs_via_braid_bits_4symbol_splited_a_less_b_mpi(Input *a_reverse, int a_size, int a_total_symbols,
                                                           Input *b, int b_size, int b_total_symbols, int threads_num) {
...
#pragma omp parallel num_threads(threads_num)  default(none) shared(bitset_left_strand_map, bitset_top_strand_map, a_reverse, b, m, n, dis_braid, total_same_length_diag, braid_ones, l_active_mask, r_active_mask)
    {
...
#pragma omp single nowait
                ...
                loop_upper_half_mpi(1, cur_diag_cube_len + 1, rev_counter, m - 1 - cur_diag_cube_len, 0,
                                    braid_ones, mask, mask_r, bitset_left_strand_map, bitset_top_strand_map, a_reverse,
                                    b);
...
#pragma omp single nowait
            ...
            loop_center_half_mpi(1, cur_diag_cube_len + 1, m - 1 - cur_diag_cube_len, 0,
                                 braid_ones, bitset_left_strand_map, bitset_top_strand_map, a_reverse, b);
        }
...
#pragma omp single nowait
                ...
                loop_upper_half_mpi(1, m, rev_counter, 0, k + 1, braid_ones,
                                    mask, mask_r, bitset_left_strand_map, bitset_top_strand_map, a_reverse, b);
...
#pragma omp single nowait
                ...
                loop_lower_half_mpi(1, m, inside_diag_num, 0, k, braid_ones,
                                    mask_prev,
                                    mask_prev_r,
                                    bitset_left_strand_map, bitset_top_strand_map, a_reverse, b);
...
#pragma omp single nowait
            ...
            loop_center_half_mpi(1, m, 0, k + 1, braid_ones, bitset_left_strand_map, bitset_top_strand_map, a_reverse,
                                 b);
...
#pragma omp single nowait
                ...
                loop_upper_half_mpi(1, m - 1, rev_counter, 0, k + 1, braid_ones,
                                    mask, mask_r, bitset_left_strand_map,
                                    bitset_top_strand_map, a_reverse, b);
...
#pragma omp single nowait
                ...
                loop_lower_half_mpi(1, m, inside_diag_num, 0, k, braid_ones, mask_prev, mask_prev_r,
                                    bitset_left_strand_map, bitset_top_strand_map, a_reverse, b);
...
#pragma omp single nowait
            ...
            loop_center_half_mpi(1, m - 1, 0, k + 1, braid_ones, bitset_left_strand_map, bitset_top_strand_map,
                                 a_reverse,
                                 b);
...
#endif //CPU_TRANSPOSITION_NETWORK_4SYMBOL_ALPHABET_BIT_H
