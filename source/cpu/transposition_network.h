...
int prefix_lcs_via_braid_mpi(std::vector<Input> a, std::vector<Input> b, int threads_num = 1) {
    ...
    #pragma omp parallel num_threads(threads_num)  default(none) shared(a, b, strand_map, left_edge, top_edge, total_same_length_diag, size, m, n, dis_braid)
    {
...
#pragma omp for schedule(static)
            for (int j = 0; j < cur_diag_len + 1; ++j) {
                bool left_strand = strand_map[left_edge + j];
                bool right_strand = strand_map[top_edge + j];
...
#pragma omp for schedule(static)
            for (int k = 0; k < m; ++k) {
                bool left_strand = strand_map[left_edge + k];
                bool right_strand = strand_map[top_edge + k];
