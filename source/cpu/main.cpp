...
int main(int argc, char *argv[]) {
    ...
    auto m = new Permutation(10,10);
    fill_permutation_matrix(m,10, 10, -2);

    auto n = new Permutation(10,10);
    fill_permutation_matrix(n,10, 10, -3);



    auto map = std::unordered_map<int, std::unordered_map<long long, std::unordered_map<long long, std::vector<std::pair<int, int>>>>>();
    semi_local_lcs::precalc(map, 5);

    auto res = Permutation(10,10);
    distance_unit_monge_product::naive::mult_dist(m,n,&res);
    auto exp = distance_unit_monge_product::steady_ant::steady_ant(m,n);
res.print(std::cout);
    std::cout<<res.is_equal_to(*exp);
    std::cout<<std::endl;
    exp->print(std::cout);
    delete exp;
    return 0;
