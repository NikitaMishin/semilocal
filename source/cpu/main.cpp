...
////
    ...
    auto mappers = encode_alphabet<int,unsigned char>(std::unordered_set<int>({1,0}));
    auto seq_a = gen_vector_seq( 16,2);
    auto seq_b = gen_vector_seq( 16,2);
//    std::vector<int> seq_a{0,0,1,0,0,0,1,0};
//    std::vector<int> seq_b{0,1,0,0,0,1,0,1};
//    a:00100010
//    b:01000101
    auto a = encode_reverse<int,unsigned char>(seq_a,&mappers.first,&mappers.second);

    auto b = encode<int, unsigned char>(seq_b,&mappers.first,&mappers.second);
...
//    std::reverse(seq_b.begin(),seq_b.end());
    ...
    std::cout<<std::endl;
    std::cout<<"a_rev:"<<std::bitset<sizeof(char)*8>(a.first.first[0])<<std::endl;
    std::cout<<"b:"<<std::bitset<sizeof(char)*8>(b.first.first[0])<<std::endl;
